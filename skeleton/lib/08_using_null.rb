...
def teachers_and_depts
  ...
  execute(<<-SQL)
    SELECT
      COALESCE(teachers.name, 'None'), COALESCE(depts.name, 'None')
    FROM
      teachers
    FULL OUTER JOIN depts ON depts.id = dept_id
  SQL
end
...
def dept_staff_counts
  ...
  execute(<<-SQL)
    SELECT
      depts.name, COALESCE(COUNT(teachers.name), 0)
    FROM
      depts
      LEFT OUTER  JOIN teachers ON depts.id = dept_id
    GROUP BY
