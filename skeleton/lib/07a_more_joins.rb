...
def expensive_tastes
  ...
  execute(<<-SQL)
  SELECT
    style, price_per_song
  FROM
    styles
  JOIN
    (SELECT
      asin, price / COUNT(song) AS price_per_song
    FROM
      albums
    JOIN
      tracks ON tracks.album = asin
    GROUP BY
      asin, price
    ORDER BY
      COUNT(song) DESC) AS tracks_per_album ON styles.album = tracks_per_album.asin

  WHERE
    price_per_song IS NOT NULL
  ORDER BY
    price_per_song DESC
  LIMIT
    5
